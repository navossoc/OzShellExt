...
if /i "%FEATURES%"=="all" (
    ...
    set DEFS=/DOZ_ENABLE_THUMBNAIL
) else if /i "%FEATURES%"=="preview" (
    set DEFS=/DOZ_ENABLE_PREVIEW
) else (
...
rc /nologo /fo build\OzShellExt.res OzShellExt.rc || exit /b 1
...
cl /nologo /std:c++17 /O2 /GL /MT /EHsc /W4 /GS /DNDEBUG /DUNICODE /D_UNICODE %DEFS% ^
   /Fo:build\ /Fd:build\ ^
   /LD OzImage.cpp ThumbnailProvider.cpp PreviewHandler.cpp ShellExt.cpp ^
   /Fe:build\OzShellExt.dll ^
   /link /DEF:exports.def /LTCG /OPT:REF /OPT:ICF /INCREMENTAL:NO ^
   build\OzShellExt.res ^
   shlwapi.lib gdi32.lib msimg32.lib ole32.lib advapi32.lib shell32.lib user32.lib ^
   windowscodecs.lib ^
   || exit /b 1

echo.
echo OK: build\OzShellExt.dll (handlers: %FEATURES%)
