// IPreviewHandler for .ozb / .ozj / .ozt files - the Explorer preview pane.
//
...
static const wchar_t* const kWindowClass = L"OzPreviewHandlerWindow";
...
class OzPreviewHandler : public IInitializeWithStream,
...
{
public:
    explicit OzPreviewHandler(ozimg::Format format) :
        m_cRef(1),
        m_stream(nullptr),
        m_format(format),
        m_site(nullptr),
    ...
    IFACEMETHODIMP QueryInterface(REFIID riid, void** ppv) override
    {
        static const QITAB qit[] = {
            QITABENT(OzPreviewHandler, IInitializeWithStream),
            QITABENT(OzPreviewHandler, IPreviewHandler),
            QITABENT(OzPreviewHandler, IPreviewHandlerVisuals),
            QITABENT(OzPreviewHandler, IOleWindow),
            QITABENT(OzPreviewHandler, IObjectWithSite),
            { nullptr, 0 },
        };
...
private:
    ...
    void DecodeOnce()
    {
        if (m_decoded || m_decodeFailed)
            return;
        m_decodeFailed = FAILED(ozimg::DecodeStream(m_stream, m_format, m_image));
    ...
    void Render(HDC hdc, const RECT& client)
    {
        ...
        if (!m_image.valid())
        {
            DrawMessage(hdc, client, L"Unable to decode this file.");
            return;
        }
        ...
        if (!m_cache || m_cacheW != dw || m_cacheH != dh)
        {
            ...
            m_cache = ozimg::CreateScaledDib(m_image, dw, dh, true);
            m_cacheW = dw;
            m_cacheH = dh;
        }
        if (!m_cache)
    ...
    {
        OzPreviewHandler* self = reinterpret_cast<OzPreviewHandler*>(
...
IUnknown* CreateOzPreviewHandler(ozimg::Format format)
{
    return static_cast<IInitializeWithStream*>(
        new (std::nothrow) OzPreviewHandler(format));
}
