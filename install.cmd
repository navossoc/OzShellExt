...
rem
rem Registers only what you ask for, for all three extensions. The two handler
rem kinds are independent, so you can install one now and add the other later
...
set DLL=OzShellExt.dll
if not exist "%DLL%" set DLL=build\OzShellExt.dll
if not exist "%DLL%" (
    echo OzShellExt.dll not found. Run build.cmd first.
    exit /b 1
)
...
regsvr32 /s /n /i:"%WHAT%" "%DEST%\OzShellExt.dll" || (
    echo regsvr32 failed. The DLL may have been built without the "%WHAT%" handler.
    exit /b 1
)

echo Registered from %DEST%\OzShellExt.dll, for .ozb .ozj .ozt:
if /i "%WHAT%"=="all"       echo   - thumbnail provider and preview handler
if /i "%WHAT%"=="thumbnail" echo   - thumbnail provider
if /i "%WHAT%"=="preview"   echo   - preview handler
echo.
echo Remaining steps:
echo  1. If another handler is installed for these extensions, turn it off.
