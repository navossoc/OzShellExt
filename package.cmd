...
rem compiler. The version comes from OzShellExt.rc, so there is one place to
rem bump it.

for /f "tokens=3" %%v in ('findstr /b /c:"#define VER_STRING" OzShellExt.rc') do set VER=%%v
set VER=%VER:"=%
if "%VER%"=="" (
    echo Could not read VER_STRING from OzShellExt.rc.
    exit /b 1
)

set NAME=OzShellExt-%VER%-x64
set STAGE=dist\%NAME%
...
rem A directory another process still has open cannot be removed, but its
rem contents can - so clear it instead of insisting on a fresh one.
if exist "%STAGE%" (
    del /q "%STAGE%\*" >nul 2>&1
) else (
...
copy /y "build\OzShellExt.dll" "%STAGE%\" >nul || exit /b 1
